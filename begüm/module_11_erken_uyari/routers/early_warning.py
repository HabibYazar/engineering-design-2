...
from module_11_erken_uyari.schemas.early_warning import (
    AlertResponse,
    AlertSummaryResponse,
    RuleCatalogResponse,
)
from module_11_erken_uyari.services import rule_engine
...
def get_alerts(
    ...
    """Aktif erken uyarı alarmlarını önem derecesine göre sıralı döndürür."""
    _validate_academic_year(db, academic_year)
    return rule_engine.evaluate(db, academic_year, severity, program_code)
...
def get_summary(
    ...
    """Alarmların önem ve kapsam bazlı özetini döndürür (yönetici paneli girdisi)."""
    _validate_academic_year(db, academic_year)
    return rule_engine.summarize(rule_engine.evaluate(db, academic_year))
