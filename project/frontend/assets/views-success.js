...
function successSourceNote(row) {
  if (!row || !row.source_type) return "";
  const synthetic = row.is_synthetic ? "SYNTHETIC_GENERATED · " : "";
  const label = row.provenance || row.source_label || row.source_type;
  return `<div class="note muted">Kaynak: ${fmt.esc(synthetic + label)}${
    row.filename ? ` · ${fmt.esc(row.filename)}` : ""}</div>`;
}
...
function successTiles(o, containerId) {
  ...
  el.insertAdjacentHTML("beforeend", successSourceNote(o));
}
