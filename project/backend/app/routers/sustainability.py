...
def get_scores(
    ...
    """Kapsamdaki programları Modül 3 verisiyle değerlendirir (puana göre artan).

    Program kapsamı seçiliyse yalnızca o programın değerlendirmesi döner.
    """
    _validate_academic_year(db, academic_year)
    return service.evaluate_all(db, academic_year, scope=resolve(db, **kapsam))
...
def get_categories(
    ...
    """Kapsamdaki programların kategori dağılımını özetler."""
    _validate_academic_year(db, academic_year)
    results = service.evaluate_all(db, academic_year, scope=resolve(db, **kapsam))
