...
def get_overview(
    ...
    """Seçili kapsamın öğrenci göstergelerini döndürür (roll-up görünüm).

    Kapsam verilmezse üniversite geneli; verilirse yalnızca o kapsamın
    programlarından toplanır.
    """
    _validate_academic_year(db, academic_year)
    return service.get_university_overview(db, academic_year, resolve(db, **kapsam))
...
def list_program_metrics(
    ...
    """Kapsamdaki programları doluluk oranına göre artan sırada listeler.

    Program kapsamı seçiliyse TEK satır döner; kardeş programlar görünmez.
    """
    _validate_academic_year(db, academic_year)
    return service.get_program_metrics(db, academic_year, resolve(db, **kapsam))
...
def get_admission_scores(
    ...
    _validate_academic_year(db, academic_year)
    return service.get_admission_score_analysis(
        db, academic_year, resolve(db, **kapsam)
    )
...
def get_demand_trends(
    ...
    """Kapsamdaki programların yıllar içindeki talep trendini döndürür."""
    return service.get_demand_trends(db, resolve(db, **kapsam))
...
def get_performance_trends(
    ...
    """Kapsamdaki programların yıllara göre ortalama GNO trendini döndürür."""
    return service.get_academic_performance_trend(db, resolve(db, **kapsam))
