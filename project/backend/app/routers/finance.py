...
def get_summary(
    ...
    Kapsam verilirse özet yalnızca o kapsamın bölüm bütçelerinden toplanır;
    üniversite geneli sayılar alt birime ait gibi gösterilmez.
    """
    return FinancialSummary(
        **service.financial_summary(db, academic_year, resolve(db, **kapsam))
    )
...
def list_department_budgets(
    ...
    Kapsam verilirse yalnızca o kapsamdaki bölümler döner — fakülte
    sayfasında başka fakültenin bölümü görünmez.
    """
    return [
        DepartmentBudgetResponse(**service.budget_to_dict(budget))
        for budget in service.list_department_budgets(
