...
def evaluate_all(
    ...
    # Kapsam, program listesini besleyen iki çağrıya da verilir; aksi
    # hâlde trend sözlüğü kapsam dışı programları taşımaya devam eder.
    trends = {t["program_code"]: t for t in analytics.get_demand_trends(db, scope)}

    results = []
    for metrics in analytics.get_program_metrics(db, academic_year, scope):
