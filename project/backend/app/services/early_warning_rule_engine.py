...
def _build_context(
    ...
    Kapsam BAĞLAM SEVİYESİNDE uygulanır: kurallar kapsam dışı bir programı
    hiç görmediği için, herhangi bir kuralın kapsamı uygulamayı unutması
    mümkün değildir. Uyarı listesi böylece seçili birime aittir.
    """
    ...
    if previous_year in available:
        previous_metrics = {
            m["program_code"]: m
            for m in analytics.get_program_metrics(db, previous_year, scope)
        }

    return {
        "academic_year": academic_year,
        "previous_academic_year": previous_year if previous_year in available else None,
        "current_metrics": analytics.get_program_metrics(db, academic_year, scope),
        "previous_metrics": previous_metrics,
        "overview": analytics.get_university_overview(db, academic_year, scope),
        "demand_trends": analytics.get_demand_trends(db, scope),
        "performance_trends": analytics.get_academic_performance_trend(db, scope),
        "sustainability": sustainability.evaluate_all(db, academic_year, scope=scope),
    }
...
def evaluate(
    ...
    config = load_rules()
    ctx = _build_context(db, academic_year, scope)
