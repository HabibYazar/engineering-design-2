...
from app.services.assistant.tool_schemas import (
    ...
    SCOPE_UNIVERSITY,
    ScopedMetric,
    AcademicStaffListInput,
    AcademicStaffListOutput,
    AcademicStaffMember,
...
def _handle_staff_list(
    db: Session, payload: AcademicStaffListInput
) -> AcademicStaffListOutput:
    """Kapsamdaki akademisyenler, performansa göre sıralı.

    Hesap YAPMAZ: mevcut `academic_staff_service.rank_staff` çıktısını
    olduğu gibi aktarır. Puanın nasıl oluştuğu (`score_breakdown`) da
    taşınır ki "bu hocanın puanı neden yüksek?" sorusu uydurma bir
    gerekçeyle değil, gerçek bileşenlerle yanıtlanabilsin.
    """
    from app.services import academic_staff_service as staff_service

    year, faculty, department, program = _resolve(db, payload)
    notes: List[str] = []
    if program is not None:
        notes.append(
            "Akademik kadro bölüm düzeyinde tutulur; program başına ayrı "
            "personel kaydı yoktur, bölüm kadrosu gösterilmiştir."
        )

    try:
        siralama = staff_service.rank_staff(
            db,
            academic_year=year,
            department_id=department.id if department else None,
            faculty_id=faculty.id if faculty else None,
        ...
        logger.exception("Akademik personel listesi alinamadi")
        return AcademicStaffListOutput(
            scope=_scope_info(year, faculty, department, program),
            notes=notes + ["Akademik personel listesi okunamadı."],
        )
    ...
    uyeler = [
        AcademicStaffMember(
            ...
            performance_score=r.get("total_score"),
            performance_band=r.get("performance_band"),
            publication_count=r.get("publication_count"),
            citation_count=r.get("citation_count"),
            project_count=r.get("project_count"),
            teaching_load_hours=r.get("teaching_load_hours"),
    ...
    return AcademicStaffListOutput(
        scope=_scope_info(year, faculty, department, program),
        staff=uyeler,
        total_count=len(siralama),
        returned_count=len(uyeler),
        notes=notes,
    )
...
registry.register(
    ToolDefinition(
        name="list_academic_staff",
        description=(
            "Seçili fakülte/bölümdeki akademisyenleri AD SOYAD, unvan, "
            "performans puanı, yayın/atıf/proje sayısı ve puan bileşenleriyle "
            "birlikte, performansa göre sıralı döndürür. "
            "'Bu bölümdeki hocalar kim?', 'hocaları performansa göre sırala', "
            "'en yüksek yayın kimde?', 'bu hocanın puanı neden yüksek?' gibi "
            "sorularda kullanılır."
        ),
        input_model=AcademicStaffListInput,
        output_model=AcademicStaffListOutput,
        handler=_handle_staff_list,
        timeout_seconds=25.0,
        required_permission="view_academic_staff",
        data_source="Akademik personel kayıtları / performans puanlaması",
    )
)
