...
class AcademicStaffListInput(ScopeInput):
    """Akademik personel listesi girdisi."""

    limit: int = Field(default=25, ge=1, le=100,
                       description="Kaç kişi döndürülsün (varsayılan 25).")


class AcademicStaffMember(BaseModel):
    """Tek bir akademisyen. Alanlar performans servisinden GELDİĞİ GİBİ."""
    ...
    teaching_load_hours: Optional[float] = None
    #: Puanı hangi bileşenlerin yükselttiği — "neden yüksek?" sorusu için.
    score_breakdown: Optional[dict] = None


class AcademicStaffListOutput(BaseModel):
    """Kapsamdaki akademisyenler, performans puanına göre azalan."""

    scope: ScopeInfo
    staff: List[AcademicStaffMember] = Field(default_factory=list)
    total_count: int = 0
