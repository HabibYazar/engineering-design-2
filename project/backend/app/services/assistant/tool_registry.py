...
class ToolDefinition:
    ...
    data_source: str
    #: Handler'a çalışan `ToolSession` de verilsin mi?
    #:
    #: Araçların hemen tamamı veritabanından okur ve oturumu bilmesine
    #: gerek yoktur. `render_chart` istisnadır: veritabanını değil,
    #: MODELİN BU TURDA ÇEKTİĞİ araç sonucunu çizer. Grafikteki sayılarla
    #: metindeki sayıların aynı sorgudan gelmesini bu sağlar.
    needs_session: bool = False

    def json_schema(self) -> Dict[str, Any]:
        """OpenAI uyumlu araç tanımı."""
        schema = self.input_model.model_json_schema()
        # OpenAI uyumlu araç şemasında $defs ve title alanları gereksiz yer
