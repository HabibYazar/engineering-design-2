...
from app.services.unit_types import UNIT_TYPES, classify_unit
...
class ResourceSpec:
    ...
    # Yalnızca "faculties" kaynağı için: dosyada "unit_type" sütunu boş
    # bırakılırsa (veya hiç yoksa) app.services.unit_types.classify_unit()
    # ile isimden otomatik türetilir. Boş bırakılırsa modelin varsayılanı
    # olan "FACULTY" sessizce atanırdı — bu, MYO/Enstitü gibi akademik
    # olmayan-fakülte birimlerini yanlış sınıflandırıp akademik
    # karşılaştırmalara sızdırırdı (bkz. unit_types.py: ACADEMIC_UNIT_TYPES).
    auto_classify_unit_type: bool = False
...
RESOURCE_SPECS: Dict[str, ResourceSpec] = {
    ...
    "faculties": ResourceSpec(
        model=Faculty,
        label="Fakülte",
        # "unit_type" opsiyoneldir: boş bırakılırsa isimden otomatik
        # türetilir (bkz. auto_classify_unit_type). Verilirse UNIT_TYPES
        # kapalı listesine (FACULTY/VOCATIONAL_SCHOOL/INSTITUTE/ADMINISTRATIVE)
        # uymak zorundadır.
        columns=["name", "code", "description", "is_active", "unit_type"],
        required=["name", "code"],
        unique_fields=("code",),
        upper_fields=("code",),
        text_fields=("name",),
...
def validate_row(
    ...
    # 8b) unit_type (yalnızca faculties): boşsa isimden otomatik türetilir,
    # doluysa kapalı listeye (UNIT_TYPES) uyup uymadığı denetlenir.
    if spec.auto_classify_unit_type:
        raw_unit_type = str(row.get("unit_type", "") or "").strip().upper()
        if raw_unit_type == "":
            cleaned["unit_type"] = classify_unit(
                cleaned.get("name", ""), cleaned.get("code", "")
            )
        elif raw_unit_type not in UNIT_TYPES:
            issues.append(
                (
                    "unit_type",
                    raw_unit_type,
                    f"unit_type alanı şu değerlerden biri olmalıdır: "
                    f"{', '.join(sorted(UNIT_TYPES))} (boş bırakılırsa isimden "
                    f"otomatik belirlenir).",
                )
            )
        else:
            cleaned["unit_type"] = raw_unit_type
