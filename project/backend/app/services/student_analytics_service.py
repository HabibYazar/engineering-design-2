...
def apply_student_scope(
    ...
    `scope` verilirse tek tek id parametrelerine EK bir süzgeç olarak
    uygulanır: kapsam içindeki program kümesi. Çağıran taraf id göndermeyi
    unutsa bile kardeş programların öğrencileri sayıma giremez.
    """
    ...
    if scope is not None and scope.program_ids is not None:
        statement = statement.where(
            Student.academic_program_id.in_(scope.program_ids)
        )
...
def _scope_from_ids(
    ...
    """Tek tek id parametrelerinden bir `Scope` üretir.

    Router'lar kapsamı zaten çözülmüş olarak geçiyor; bu yardımcı,
    servisi doğrudan çağıran testler ve iç kullanımlar için var. Import
    fonksiyon içinde yapılır çünkü `scope` modülü bu modülü değil, bu
    modül onu kullanıyor — modül düzeyinde import döngü kurardı.
    """
    from app.services.scope import resolve

    return resolve(db, faculty_id, department_id, academic_program_id)
...
def build_overview(
    ...
    # RESMÎ ÖĞRENCİ SAYISI. Bireysel öğrenci kaydı bulunmayan gerçek
    # veride `Student` tablosu boştur; toplam, ÖSYM yerleştirme
    # kayıtlarından türetilir (bkz. app/services/student_count.py).
    # Öğrenci satırı varsa o sayılır ve davranış değişmez.
    resmi_toplam = student_count.total_for_scope(
        db,
        scope if scope is not None
        else _scope_from_ids(db, faculty_id, department_id, academic_program_id),
    )
    total: int = (
        int(row.total) if row.total
        else (resmi_toplam if resmi_toplam is not None else 0)
    )
    ...
    record_statement = apply_student_scope(
        record_statement, faculty_id, department_id, academic_program_id,
        enrollment_year, scope=scope,
    )
    if academic_year:
    ...
    return StudentOverview(
        ...
        # Uygulanan filtreler ŞEFFAF olarak döner: arayüz hangi kapsamda
        # olduğunu başlıkta doğru yazabilsin ve testler filtrenin gerçekten
        # uygulandığını doğrulayabilsin diye. Kapsamdan türeyen değerler de
        # yazılır; yoksa "filtre yok" gibi görünürdü.
        applied_filters={
            "faculty_id": faculty_id or (scope.faculty_id if scope else None),
            "department_id": department_id or (scope.department_id if scope else None),
            "academic_program_id": (
                academic_program_id
                or (scope.academic_program_id if scope else None)
            ),
...
def build_program_analytics(
    ...
    `scope`, tek tek id parametrelerine EK bir süzgeçtir ve her zaman
    uygulanır. Böylece bir çağıran `academic_program_id` göndermeyi
    unutsa bile program kapsamında kardeş programlar listeye giremez.
    Ayrıca üniversite kapsamında İDARİ birimler (Rektörlük) dışarıda
    kalır: onların programı yoktur, akademik karşılaştırmaya girmezler.
    """
    ...
    if scope is not None and scope.program_ids is not None:
        program_statement = program_statement.where(
            AcademicProgram.id.in_(scope.program_ids)
        )
    else:
        # Üniversite kapsamı: akademik olmayan birimler karşılaştırmaya girmez.
        program_statement = program_statement.where(
    ...
    # RESMÎ ÖĞRENCİ SAYISI — tek kaynak (bkz. app/services/student_count.py).
    # Bireysel öğrenci kaydı olmayan gerçek veride sayı ÖSYM yerleştirme
    # kayıtlarından türetilir; öğrenci satırı varsa onlar sayılır.
    resmi_sayilar = student_count.program_counts(db, scope)
    ...
    for prog_id, prog_name, prog_code, dept_id, dept_name, fac_id, fac_name in programs:
        ...
        resmi = resmi_sayilar.get(prog_id)
        # Resmî sayı varsa toplam öğrenci odur. Öğrenci satırı yoksa
        # `agg` None'dır ve eski kod 0 yazıyordu — bu, gerçek veride
        # bütün programları "0 öğrenci" gösteriyordu.
        total: int = (
            resmi.student_count if resmi and resmi.student_count is not None
            else (int(agg.total) if agg else 0)
        )
        # Aktif öğrenci: öğrenci satırı varsa durumundan gelir. ÖSYM
        # verisinde durum bilgisi YOKTUR; son 4 kohortun tamamı hâlen
        # kayıtlı öğrenci gövdesidir, bu yüzden aktif = toplam.
        active: int = int(agg.active) if agg else total
...
def build_department_analytics(
    ...
    # böylece aynı SQL sorguları ikinci kez çalıştırılmıyor. Kapsam da
    # program katmanında uygulandığı için buraya kendiliğinden taşınır.
    programs = build_program_analytics(
        db, faculty_id=faculty_id, department_id=department_id,
        academic_year=academic_year, scope=scope,
    )
...
def build_faculty_analytics(
    ...
    Üniversite kapsamında yalnızca AKADEMİK birimler görünür; Rektörlük
    gibi idari birimler fakülte karşılaştırmasına girmez.
    """
    departments = build_department_analytics(
        db, faculty_id=faculty_id, academic_year=academic_year, scope=scope
    )
