...
.PARAMETER SkipSeed
    Veri yüklemeyi atlar. Veritabanı zaten doluysa kullanılır.
...
.EXAMPLE
    .\run_project.ps1 -SkipInstall -SkipSeed
...
$BackendDir  = Join-Path $RootDir "integration\backend"
$FrontendDir = Join-Path $RootDir "integration\frontend"
...
try {
    ...
    if ($SkipSeed) {
        Write-Warn2 "-SkipSeed verildi, veri yukleme atlandi."
    } else {
        Write-Ok "Ortak demo verisi yukleniyor (idempotent)..."
