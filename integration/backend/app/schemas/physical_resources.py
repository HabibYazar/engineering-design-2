...
class PhysicalFacilityResponse(PhysicalFacilityBase):
    ...
    occupancy_percent: float = Field(examples=[87.5])
...
class UtilizationByTypeItem(BaseModel):
    ...
    total_capacity: int = Field(examples=[180])
    total_occupied: int = Field(examples=[130])
    average_utilization_percent: float = Field(examples=[72.22])
...
class AllocationByDepartmentItem(BaseModel):
    ...
    total_capacity: int = Field(examples=[80])
...
class SpacePerPersonResponse(BaseModel):
    ...
    total_capacity: int = Field(examples=[452])
...
class FacilityFlagItem(BaseModel):
    ...
    capacity: int = Field(examples=[50])
    occupied: int = Field(examples=[22])
    occupancy_percent: float = Field(examples=[44.0])
...
class CapacityOverview(BaseModel):
    ...
    total_capacity: int = Field(examples=[452])
    total_occupied: int = Field(examples=[329])
    overall_occupancy_percent: float = Field(examples=[72.79])
    underutilized_count: int = Field(examples=[2])
    overcrowded_count: int = Field(examples=[3])
